...
export const createUser = async (req, res) => {
    ...
    try {
        ...
        const user = new UserModel({ email, fullName, password: hashedPass, phone })
        await user.save();
        res.status(201)
            .send({
                success: true,
...
export const loginUser = async (req, res) => {
    try {
        ...
        const token = jwt.sign({ _id: userData._id,role: userData.role }, process.env.SECRET_KEY, {
            expiresIn: '1d'
        });
        res.status(200)
            .send({
...
export const addSkill = async (req, res) => {
    try {
        ...
        proficiency = proficiency.charAt(0).toUpperCase() + proficiency.slice(1).toLowerCase();
        ...
        res.status(500).send({ success: false, message: "Error adding skill", error: error.message });
    }
};
...
export const addEducation = async (req, res) => {
    try {
        ...
        res.status(500).send({ success: false, message: "Error adding education", error: error.message });
    }
};
...
export const addExperience = async (req, res) => {
    try {
        ...
        res.status(500).send({ success: false, message: "Error adding experience", error: error.message });
    }
};
...
export const getAppliedJobsByUserId = async (req, res) => {
    try {
        ...
        const appliedJobs = await ApplicationModel.find({ userId }).populate('jobId');

        if (appliedJobs.length === 0) {
            return res.status(404).send({ success: false, message: "No applied jobs found for this user" });
        }
...
export const addResume = async (req, res) => {
    try {
        ...
        if (!file) {
            return res.status(400).send({ success: false, message: 'No resume file uploaded' });
        }
        ...
        res.status(500).send({ success: false, message: 'Error uploading resume', error: error.message });
    }
};
...
export const removeResume = async (req, res) => {
    try {
        ...
        await cloudinary.uploader.destroy(`user_resumes/${publicId}`, { resource_type: 'raw' });
